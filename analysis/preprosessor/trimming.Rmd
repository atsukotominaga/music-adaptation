...
```{r}
dt_ioi_subset <- dt_ioi[Articulation != "NA" & !is.na(IOI)]

gghistogram(dt_ioi_subset, x = "IOI", color = "Articulation", bins = 150)
```
...
dt_ioi_subset[Articulation == "LtoS" | Articulation == "StoL"]$Boundary <- "Yes"
...
for (phrase in 1:length(ls_legato)){
  for (note in 1:length(ls_legato[[phrase]])){
    dt_kot[Interval == ls_legato[[phrase]][note]]$Articulation <- "Legato"
    dt_kot[Interval == ls_legato[[phrase]][note]]$Dynamics <- "Forte"
  }
}
# Staccato/Piano
for (phrase in 1:length(ls_staccato)){
  for (note in 1:length(ls_staccato[[phrase]])){
    dt_kot[Interval == ls_staccato[[phrase]][note]]$Articulation <- "Staccato"
    dt_kot[Interval == ls_staccato[[phrase]][note]]$Dynamics <- "Piano"
  }
}

# assign Subcomponent Change
for (number in change_1){
  dt_kot[Interval == number]$Articulation <- "LtoS"
  dt_kot[ Interval == number]$Dynamics <- "FtoP"
}
for (number in change_2){
  dt_kot[Interval == number]$Articulation <- "StoL"
  dt_kot[Interval == number]$Dynamics <- "PtoF"
}
```
...
```{r}
dt_kot_subset <- dt_kot[Articulation != "NA" & !is.na(KOT)]

gghistogram(dt_kot_subset, x = "KOT", color = "Articulation", bins = 150)
```

### After removing outliers
```{r}
kot_subcomponent <- dt_kot_subset[, .(N = .N, Mean = mean(KOT), SD = sd(KOT)), by = Articulation]
dt_kot_trimmed <- data.table()
for (subcomponent in unique(dt_kot_subset$Articulation)){
  upper <- kot_subcomponent[Articulation == subcomponent]$Mean+3*kot_subcomponent[Articulation == subcomponent]$SD
  lower <- kot_subcomponent[Articulation == subcomponent]$Mean-3*kot_subcomponent[Articulation == subcomponent]$SD
  dt_current <- dt_kot_subset[Articulation == subcomponent & KOT < upper & KOT > lower]
  dt_kot_trimmed <- rbind(dt_kot_trimmed, dt_current)
}

gghistogram(dt_kot_trimmed, x = "KOT", color = "Articulation", bins = 150)
```
...
for (phrase in 1:length(ls_legato2)){
  for (note in 1:length(ls_legato2[[phrase]])){
    dt_vel[RowNr == ls_legato2[[phrase]][note]]$Articulation <- "Legato"
    dt_vel[RowNr  == ls_legato2[[phrase]][note]]$Dynamics <- "Forte"
  }
}
# Staccato/Piano
for (phrase in 1:length(ls_staccato2)){
  for (note in 1:length(ls_staccato2[[phrase]])){
    dt_vel[RowNr  == ls_staccato2[[phrase]][note]]$Articulation <- "Staccato"
    dt_vel[RowNr  == ls_staccato2[[phrase]][note]]$Dynamics <- "Piano"
  }
}
```
...
### Before removing outliers
```{r}
dt_vel_subset <- dt_vel[Articulation != "NA" & !is.na(Velocity)]

gghistogram(dt_vel_subset, x = "Velocity", color = "Articulation", bins = 150)
```

### After removing outliers
```{r}
vel_subcomponent <- dt_vel_subset[, .(N = .N, Mean = mean(Velocity), SD = sd(Velocity)), by = Articulation]
dt_vel_trimmed <- data.table()
for (subcomponent in unique(dt_vel_subset$Articulation)){
  upper <- vel_subcomponent[Articulation == subcomponent]$Mean+3*vel_subcomponent[Articulation == subcomponent]$SD
  lower <- vel_subcomponent[Articulation == subcomponent]$Mean-3*vel_subcomponent[Articulation == subcomponent]$SD
  dt_current <- dt_vel_subset[Articulation == subcomponent & Velocity < upper & Velocity > lower]
  dt_vel_trimmed <- rbind(dt_vel_trimmed, dt_current)
}

gghistogram(dt_vel_trimmed, x = "Velocity", color = "Articulation", bins = 150)
```
...
for (phrase in 1:length(ls_legato)){
  for (note in 1:length(ls_legato[[phrase]])){
    dt_vel_diff[Interval == ls_legato[[phrase]][note]]$Articulation <- "Legato"
    dt_vel_diff[Interval == ls_legato[[phrase]][note]]$Dynamics <- "Forte"
  }
}
# Staccato/Piano
for (phrase in 1:length(ls_staccato)){
  for (note in 1:length(ls_staccato[[phrase]])){
    dt_vel_diff[Interval == ls_staccato[[phrase]][note]]$Articulation <- "Staccato"
    dt_vel_diff[Interval == ls_staccato[[phrase]][note]]$Dynamics <- "Piano"
  }
}

# assign Subcomponent Change
for (number in change_1){
  dt_vel_diff[Interval == number]$Articulation <- "LtoS"
  dt_vel_diff[ Interval == number]$Dynamics <- "FtoP"
}
for (number in change_2){
  dt_vel_diff[Interval == number]$Articulation <- "StoL"
  dt_vel_diff[Interval == number]$Dynamics <- "PtoF"
}
...
### Before removing outliers
```{r}
dt_vel_diff_subset <- dt_vel_diff[Articulation != "NA" & !is.na(Diff)]

gghistogram(dt_vel_diff_subset, x = "Diff", color = "Articulation", bins = 150)
```

### After removing outliers
```{r}
vel_diff_subcomponent <- dt_vel_diff_subset[, .(N = .N, Mean = mean(Diff), SD = sd(Diff)), by = Articulation]
dt_vel_diff_trimmed <- data.table()
for (subcomponent in unique(dt_vel_diff_subset$Articulation)){
  upper <- vel_diff_subcomponent[Articulation == subcomponent]$Mean+3*vel_diff_subcomponent[Articulation == subcomponent]$SD
  lower <- vel_diff_subcomponent[Articulation == subcomponent]$Mean-3*vel_diff_subcomponent[Articulation == subcomponent]$SD
  dt_current <- dt_vel_diff_subset[Articulation == subcomponent & Diff < upper & Diff > lower]
  dt_vel_diff_trimmed <- rbind(dt_vel_diff_trimmed, dt_current)
}

gghistogram(dt_vel_diff_trimmed, x = "Diff", color = "Articulation", bins = 150)
```
