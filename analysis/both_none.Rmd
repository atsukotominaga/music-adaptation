---
title: 'Comparison between both and none categories'
output:
...
Description: This is a summary of analysis comparing between both and none categories.
...
```{r ioi}
# for each individual
ioi_trial <- dt_ioi[Category == "none" | Category == "both", .(N = .N, Mean = mean(IOI), SD = sd(IOI)), by = .(SubNr, TrialNr, Stimuli, Articulation, Dynamics, Category)]
...
ioi_seq <- dt_ioi[Category == "none" | Category == "both", .(N = .N, Mean = mean(IOI), SD = sd(IOI)), by = .(SubNr, Category, Interval)]
...
```{r kot, echo = FALSE}
# for each individual
dt_subset_kot <- dt_kot[Subcomponent1 == "Legato" | Subcomponent1 == "Staccato"]
kot_trial <- dt_subset_kot[Category == "none" | Category == "both", .(N = .N, Mean = mean(KOT), SD = sd(KOT)), by = .(SubNr, TrialNr, Stimuli, Articulation, Dynamics, Category, Subcomponent1)]
...
kot_seq <- dt_kot[Category == "none" | Category == "both", .(N = .N, Mean = mean(KOT), SD = sd(KOT)), by = .(SubNr, Category, Interval)]
...
```{r vel, echo = FALSE}
# for each individual
dt_subset_vel <- dt_vel[Subcomponent2 != "NA"]
vel_trial <- dt_subset_vel[Category == "none" | Category == "both", .(N = .N, Mean = mean(Velocity), SD = sd(Velocity)), by = .(SubNr, TrialNr, Stimuli, Articulation, Dynamics, Category, Subcomponent2)]
...
vel_seq <- dt_vel[Category == "none" | Category == "both", .(N = .N, Mean = mean(Velocity), SD = sd(Velocity)), by = .(SubNr, Category, RowNr)]
...
# for each individual
dt_subset_vel_diff <- dt_vel_diff[Subcomponent2 == "FtoP" | Subcomponent2 == "PtoF"]
vel_diff_trial <- dt_subset_vel_diff[Category == "none" | Category == "both", .(N = .N, Mean = mean(Diff), SD = sd(Diff)), by = .(SubNr, TrialNr, Stimuli, Articulation, Dynamics, Category, Subcomponent2)]
...
vel_diff_seq <- dt_vel_diff[Category == "none" | Category == "both", .(N = .N, Mean = mean(Diff), SD = sd(Diff)), by = .(SubNr, Category, Interval)]
