...
```{r vel, echo = FALSE}
# for each individual
vel_trial <- dt_vel[Dynamics != "NA", .(N = .N, Mean = mean(Velocity), SD = sd(Velocity)), by = .(SubNr, TrialNr, Stimuli, Articulation, Dynamics, Category, Subcomponent2)]

vel <- vel_trial[, .(N = .N, Mean = mean(Mean), SD = sd(Mean)), by = .(SubNr, Articulation, Dynamics, Category, Subcomponent2)]
setorder(vel, "SubNr", "Subcomponent2", "Dynamics", "Articulation", "Category")
vel
```
