...
Description: This is a rough overview of non-expressive performance (Duration) from teaching-v2.0.
...
excluded_duration <- duration[Mean < duration_all$Mean+3*duration_all$SD & Mean > duration_all$Mean-3*duration_all$SD]
print(excluded_duration)
```
- The number of excluded performance: `r duration_all$N-nrow(excluded_duration)`
...
cv <- dt_duration[EighthNote == "Yes", .(N = length(normDu), CV = sd(normDu)/mean(normDu)), by = .(SubNr, TrialNr)]
print(cv)
```
