...
# Group mean
ioi_all <- dt_ioi[EighthNote == "Yes", .(N = length(normIOI), Mean = mean(normIOI), SD = sd(normIOI))]
print(ioi_all)
```
...
cv <- dt_ioi[EighthNote == "Yes", .(N = length(normIOI), Mean = mean(normIOI), SD = sd(normIOI), CV = sd(normIOI)/mean(normIOI)), by = .(SubNr, TrialNr)]
cv
```
...
# export dt_ioi
write.csv(dt_ioi, file = "./data_ioi_onset.csv", row.names = F)
```
