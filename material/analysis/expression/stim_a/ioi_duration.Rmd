...
duration_seq <- dt_duration[Subcomponent == "Legato", .(N = length(normDu), Mean = mean(normDu)), .(SubNr, TrialNr, RowNr)]

p_duration_seq_leg <- ggplot(data = duration_seq, aes(x = RowNr, y = Mean, group = TrialNr, colour = TrialNr)) +
  geom_line() +
  geom_point() +
  facet_grid(SubNr ~ .) +
...
p_duration_dis_sta <- ggplot(dt_duration[Subcomponent == "Staccato"], aes(x = normDu)) +
  geom_histogram(binwidt = .1, alpha = .5, position="identity") +
  facet_grid(TrialNr ~ SubNr) +
