...
```{r supplementary, out.width = "100%", fig.cap = "\\label{fig:supplementary}Schematic representation of stimuli generation processes", echo = FALSE}
here::i_am("README.md")
stim <- image_border(image_read(here("paper/image", "supplementary.png")), "#FFFFFF", "140x140")
image_write(stim, path = here("paper/image", "Fig10.tiff"), format = "tiff")
stim
```
...
kot_profiles <- gghistogram(dt_kot[SubArt != "NA"], x = "FixedKOT", color = "SubArt", facet.by = "MidFile", bins = 50, rug = TRUE)
...
```{r plot-s-ioi, out.width = "100%", fig.cap = "\\label{fig:s-ioi}A"}
img_s_ioi_profiles
```

```{r plot-s-kot, out.width = "100%", fig.cap = "\\label{fig:s-kot}A"}
img_s_kot_profiles
```

```{r plot-s-vel, out.width = "100%", fig.cap = "\\label{fig:s-vel}A"}
img_s_vel_profiles
```

```{r plot-s-vel-diff, out.width = "100%", fig.cap = "\\label{fig:s-vel-diff}A"}
