...
There are possibilities that it may be difficult in general for experts to flexibly adjust to different action parameters while teaching or they might have chosen to focus on the most salient parameters or the ones that they find most relevant. For example, when teaching dance, it is considered that good teachers need to identify and build the basic knowledge and skills first and then add complexity progressively from the basic foundation [@mainwaring_2010]. For example, when teaching social dance, a teacher started with a basic step as the hustle and then moved onto more complex steps progressively like the waltz and the cha cha [@lobel_2021]. Karl Leimer, a German music teacher and pianist, wrote in his booked that when doing finger exercises, students should perform with legato at first [@gieseking_1972]. Legato creates round and mellow tones, which is one of the most essential elements of piano touches in performance. In both dance and music examples, the ultimate goal of skill acquisition is that students are able to use multiple techniques flexibly while performing. However, it seems that students need to build complex skills based on basic ones during the learning process. Therefore, identifying and focusing on the most relevant problem for each student may be more effective to facilitate learning.
...
We aimed to generate four types of recordings and each type (i.e., both, art only, dyn only, none) had four instances (see *Apparatus and stimuli*) by using the performance data from @tominaga_2022. First, 16 instances for each type were generated. Second, for each type, we chose the four best instances, which exhibited the characteristic of each type well.

### 2. Generation of 16 instances per each type

In order to create recordings, we needed information about 1) the pitch order, 2) when each note was pressed (related to tempo), 3) when each note was released (related to articulation) and 4) how fast each note was hit (related to dynamics). Pitch information was already determined and the same for all structures because of the sheet music (*Fig \ref{fig:stim}*). 
